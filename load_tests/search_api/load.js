...
export const options = {
  stages: [
    ...
    { duration: "30s", target: 20 }, // Ramp-up to 300 users
    { duration: "1m", target: 30 }, // Stay at 500 users for 1 minute
...
function getSearchTerm() {
  ...
  if (Math.random() < 0.8) {
    return popularSearchTerms[
      Math.floor(Math.random() * popularSearchTerms.length)
    ];
  } else {
    // 20% of the time, use a long-tail search term
    return longTailSearchTerms[
      Math.floor(Math.random() * longTailSearchTerms.length)
    ];
  }
}
...
export default function () {
  ...
  //http://54.255.149.216:8000/products?query=${encodeURIComponent(searchTerm)}&query_type=dense&limit=5
  const url = `http://54.255.149.216:8000/products?query=${encodeURIComponent(
    searchTerm
  )}&query_type=dense&limit=5`;
